...
class Address(Base):
    ...
    # Constraints:
	__table_args__ = (
        ForeignKeyConstraint(["user_id"], ["users.user_id"], name="fk_address_user"),
    )
...
class UserItem(Base):
	...
	__table_args__ = (
		ForeignKeyConstraint(
			["user_id"],
			["users.user_id"],
			name="fk_user_item_user",
		),
		ForeignKeyConstraint(
			["product_id"],
			["products.id"],
			name="fk_user_item_product",
		),
	)
...
class Product(Base):
	...
	imagePath = Column(String(32))
	
	# Product Constructor:
	def __init__(self, title, descrip, category, price, stock, meadrating, imagePath):
		self.title = title
